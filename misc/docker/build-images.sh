...
for dockerfile in Dockerfile.arm32v7-raspbian:* Dockerfile.arm32v7-steamlink:*; do
    ...
    docker build --progress=plain --platform linux/arm/v7 -t "$DOCKER_IMAGE_BASE/$image_name" -f "$dockerfile" .
done
...
for dockerfile in Dockerfile.arm64v8-*; do
    [ -f "$dockerfile" ] || continue
    image_name="`echo "$dockerfile" | sed 's/Dockerfile\.\(.*\)/\1/'`"
    docker build --progress=plain --platform linux/arm64 -t "$DOCKER_IMAGE_BASE/$image_name" -f "$dockerfile" .
done